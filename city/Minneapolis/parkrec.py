...
class MplsParkRecEventScraper(Scraper):
    ...
    def scrape(self):
        for m in mtgs:
            ...
            if d['title'] == 'Board Meeting':
                ...
                event_title = d['title']
            e = Event(name=event_title,start_date=dt,location_name=d['location'], description=d['zfo'], classification='govt')
            if d['title'] == 'Board Meeting':
                e.add_committee('Minneapolis Parks and Recreation')
            e.add_source(d['link'])
            yield e
